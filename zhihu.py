...
def index():
    ...
    f = open('/Users/zhangjie/Downloads/zhihu-terminal-1.0/zhihu.html', 'w')
    f.write(soup.prettify())
    f.close()
    items = soup.select(".feed-item")
    # print 'soup2:',len(items)
    index=0
    for item in items:
        #tid, t, l = get_item_info_another(item)
        from TimeLine import TLItem
        f = open('/Users/zhangjie/Downloads/zhihu-terminal-1.0/zhihu'+str(index)+'.html', 'w')
        f.write(item.prettify("ISO8859-2"))
        f.close()
        try:
